...
fun `1654-랜선 자르기`(){
    ...
    var low = 1L
    val arr = LongArray(k){
        val element = br.readLine().toLong()
        if(high < element) high = element
        element
    }
    ...
    while(low <= high){
        ...
        /*
        * n 만큼 개수를 채우지 못한다면 high 를 줄이기
        * n 을 넘게 채웠어도 최대 길이로 나눈게 아닐 수 있으므로 low 를 늘려주기
        * */
        if(count < n){
            high = mid - 1
        } else {
