...
/*
* 적어도 M 미터의 나무를 가져야하므로 M 미만은 불가
* M <= 나무의 높이
* */
fun `2805-나무 자르기`(){
    val br = System.`in`.bufferedReader()
    val bw = System.out.bufferedWriter()
    val inputToken = StringTokenizer(br.readLine())
    val n = inputToken.nextToken().toInt()
    val m = inputToken.nextToken().toLong()
    var high = Long.MIN_VALUE
    var low = 1L
    var result = 0L
    val treeToken = StringTokenizer(br.readLine())
    val treeHeightArr = LongArray(n){
        val height = treeToken.nextToken().toLong()
    ...
    while (low <= high){
        ...
        for(treeHeight in treeHeightArr){
            if(mid < treeHeight){
                sliceHeight += treeHeight - mid
            }
        }

        // m 이상이면 low 를 늘려서 최대한 짧게 자르도록 설정
        if(sliceHeight >= m){
            low = mid + 1
            result = mid
        } else {
