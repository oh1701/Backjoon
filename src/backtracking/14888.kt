...
fun `14888-연산자 끼워넣기`() {
    ...
    n = br.readLine().toInt()
    val numbersToken = StringTokenizer(br.readLine())
    val operatorToken = StringTokenizer(br.readLine())
    numbers = List(n){ numbersToken.nextToken().toInt() }
    operators = IntArray(4){ operatorToken.nextToken().toInt() }
...
private fun dfs(currentValue: Int, index: Int) {
    if(index == n){
    ...
    for(i in 0 .. 3){
        if(operators[i] > 0){
            ...
            val nextValue = when(i){
                0 -> currentValue + nextNumber // 더하기
                1 -> currentValue - nextNumber // 빼기
                2 -> currentValue * nextNumber // 곱셈
                3 -> currentValue / nextNumber // 나눗셈
