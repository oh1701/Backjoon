...
fun `13305-주유소`(){
    ...
    // 도시 거리
    val distanceToken = StringTokenizer(br.readLine())
    val distanceArr = LongArray(n - 1){ distanceToken.nextToken().toLong() }
    // 기름
    val costToken = StringTokenizer(br.readLine())
    val costArr = LongArray(n){ costToken.nextToken().toLong() }

    var totalCosts = 0L
    var minCost = costArr[0]

    // 마지막 도시에서는 주유하지 않으므로 - 2
    for(i in 0 until n - 1){
        minCost = kotlin.math.min(minCost, costArr[i])
        totalCosts += minCost * distanceArr[i]
    }

    bw.write(totalCosts.toString())
    bw.flush()
    bw.close()
    br.close()
}