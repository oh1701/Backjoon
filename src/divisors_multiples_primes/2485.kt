...
fun `2485-가로수`(){
    ...
    // 위치간의 간격을 구하여 리스트로 형성
    val arr = List(size){ br.readLine().toInt() }.zipWithNext { a, b -> b - a }
    // 첫 인덱스부터 결과값을 누적시키며 최대공약수를 구함
    val spacing = arr.reduce(::gcd)
    /* 간격 차이 / 최대공약수 (일정 간격) 만약 위치가 5, 15라 10 차이가 나고, 일정 간격이 5라면 사이에는 10 하나가 있어야하므로
    * 간격 차이 / 간격 - 1 하여 1개
    * */
    val toAdd = arr.sumOf { it / spacing - 1 }
